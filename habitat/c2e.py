...
def c2e(cubemap, h, w, mode='bilinear', cube_format='dice'):
    ...
    if cube_format == 'horizon':
        ...
        cubemap = utility.cube_list2h(cubemap)
    elif cube_format == 'dict':
        cubemap = utility.cube_dict2h(cubemap)
    elif cube_format == 'dice':
        cubemap = utility.cube_dice2h(cubemap)
    else:
    ...
    uv = utility.equirect_uvgrid(h, w)
    ...
    tp = utility.equirect_facetype(h, w)
    coor_x = np.zeros((h, w))
    coor_y = np.zeros((h, w))
    ...
    equirec = np.stack([
        utility.sample_cubefaces(cube_faces[..., i], tp, coor_y, coor_x, order=order)
